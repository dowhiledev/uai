...
def test_custom_adapter_chat_session():
    ...
    with _temp_env(KOSMOS_TOML=os.path.join('examples', 'custom_adapter', 'kosmos.toml'), UAI_PROCRASTINATE_INLINE="1"):
        ...
        assert any(m.get('role') == 'assistant' and 'echo:' in (m.get('content') or '') for m in msgs)
        ...
        assert any(m.get('role') == 'assistant' and '(n=2)' in (m.get('content') or '') for m in msgs)
